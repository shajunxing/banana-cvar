# A minimal, fast, transparent, robust dynamic extension for C language that supports GC and is compatible with Json data types
...
2. Assign a variable to B variable, the actual pass is the address, in order to save memory, null/true/false are global constants, all the primitive values of number/string are to create new variables, not to modify the primitive variables, array/object has another set of functions for addition, deletion and modification.
...
|-|-|-|
|dump()||Prints debug information for all constants, variables, and references|
|void gc()||Garbage collection|
|vassign(a, b)|a = b|Assignment|
|vdeclare(a, b)|var a = b|Assign value while declaring variable|
...
Null values are variables of type vtnull, not NULL, and are stored as constants to save memory.

|Functions/Macros|Dynamic Syntax|Descriptions/Examples|
|-|-|-|
|struct var *znew()||Returns an address that points to a null constant |
|zdeclare(a)|var a = null|Declare variable and assign null value|

## Boolean

To save memory, booleans are also stored as constants.

|Functions/Macros|Dynamic Syntax|Descriptions/Examples|
|-|-|-|
|struct var *bnew(bool b)||Returns an address that points to the true/false constant |
|bdeclare(a, b)|var a = true|Declare variable and assign bool value<br>bdeclare(a, true)|
|bool bvalue(struct var *pv)||Returns the primitive value of the boolean variable|

## Number

Numeric values are numbers, and they do not distinguish between integers and floating-point numbers, and can be replaced with other distinguishable or high-precision numerical computing libraries if necessary, but do not distinguish at the API level.

|Functions/Macros|Dynamic Syntax|Descriptions/Examples|
|-|-|-|
|struct var *nnew(double n)||Create the number variable|
|ndeclare(a, b)|var a = 3.14|Declare variable and assign numeric value<br>ndeclare(a, 3.14)|
|double nvalue(struct var *pv)||Returns the primitive value of the number variable|
...
|sdeclare(a, b)|var a = "hello"|Declare variable and assign string<br>sdeclare(a, "hello")|
|char *svalue(struct var *pv)||Returns the primitive value of the string variable (pointing to an array of characters ending in 0)|
|size_t slength(struct var *pv)|a.length()|returns string length|
|struct var *sconcat(size_t num, ...)|var a = "hi"<br>var b = "all"<br>var c = a + b|Concatenate one or more strings<br>sdeclare(a, "hi")<br>sdeclare(b, "all")<br>vdeclare(c, sconcat(2, a, b))|
...
|adeclare(a)|var a = []|Declare variable and assign empty array|
|void aclear(struct var *pv)|v.clear()|Cleanup array|
...
|void aput(struct var *pv, size_t idx, struct var *pval)|v[idx] = val|array specifies that the subscript is written to replace the primitive value, and the program exits if the subscript is out of bounds |
...
|void asort(struct var *pv, acomp_t comp)|v.sort()|array sorting, comp is a user-specified sort function in the format int (*acomp_t)(const struct var *, const struct var *) If it is NULL, the default sorting function is called, and the rules are: 1. If the type is different, according to the order of null, boolean, number, string, array, and object; 2. If the type is same, false before true, and the string is called memcmp with the minimum length + 1, that is, the  at the end of the  \0 to participate in the comparison, array and object only compare the number of elements contained in |
...
|odeclare(a)|var a = {}|Declare variable and assign empty object|
|void oclear(struct var *pv)|v.clear()|Cleanup object|
