...
static void toj(struct stringbuffer *psb, struct varbuffer *pvb, struct var *variable) {
    vbpush(pvb, variable);
    ...
    case vtnull:
        ...
        break;
    case vtboolean:
        sbappend(psb, variable->bvalue ? "true" : "false");
        break;
    case vtnumber:
        // 自己写，取代asprintf
        // 为了防止数值溢出，不用整型而是用double类型，以及使用math函数计算而非强制类型转换
        double num = variable->nvalue;
        if (num < 0) {
            sbappend(psb, "-");
            num = -num;
        }
    ...
    case vtstring:
        sbappend(psb, "\"");
        esc(psb, (variable->svalue).base, (variable->svalue).length);
        sbappend(psb, "\"");
        break;
    case vtarray:
        sbappend(psb, "[");
        for (size_t i = 0; i < (variable->avalue).length; i++) {
            // 防止套娃
            if (vbfind(pvb, (variable->avalue).base[i]) > -1) {
                continue;
            }
            if (i > 0) {
                sbappend(psb, ",");
            }
            toj(psb, pvb, (variable->avalue).base[i]);
        }
        sbappend(psb, "]");
        break;
    case vtobject:
        sbappend(psb, "{");
        size_t j = 0;
        for (size_t i = 0; i < (variable->ovalue).capacity; i++) {
            // 注意value == NULL是已删除状态
            if ((variable->ovalue).base[i].key.base == NULL || (variable->ovalue).base[i].value == NULL) {
                continue;
            }
            // 防止套娃
            if (vbfind(pvb, (variable->ovalue).base[i].value) > -1) {
                continue;
            }
            if (j > 0) {
                sbappend(psb, ",");
            ...
            j++;
            sbappend(psb, "\"");
            esc(psb, (variable->ovalue).base[i].key.base, (variable->ovalue).base[i].key.length);
            sbappend(psb, "\":");
            toj(psb, pvb, (variable->ovalue).base[i].value);
        }
        sbappend(psb, "}");
        break;
    }
...
// 转换为JSON兼容的字符串格式
struct var *vtojson(struct var *variable) {
    exitif(variable == NULL);
...
const char *tojson(struct var *variable) {
    return svalue(vtojson(variable));
}
